            ...
            <form>
                <md-card>
                    <md-card-header data-background-color="green">
                        <h4 class="title">Add Child Sub Menu</h4>
                    </md-card-header>
                    ...
                    <md-card-content>
                        <div class="md-layout">
                            <div class="md-layout-item md-small-size-100 md-size-50">
                                <md-field>
                                    <label>Child Sub Menu Name</label>
                                    <md-input v-model="sub_menu_name"></md-input>
                                </md-field>
                            </div>
                            <div class="md-layout-item md-small-size-100 md-size-50">
                                <md-field>
                                    <label for="movie">Sub Menu</label>
                                    <md-select v-model="sub_menu_id">
                                        <md-option v-for="item in sub_menu_list" v-bind:key="item" :value="item.id">{{item.sub_menu_name}}</md-option>
                                    </md-select>
                                </md-field>
                            </div>
                            <div class="md-layout-item md-small-size-100 md-size-50">
...
export default {
    ...
    methods: {
        addEditSubMenu() {
            ...
            formData.append('child_sub_menu_name', this.child_sub_menu_name)
            formData.append('sub_menu_id', this.sub_menu_id)
            ...
            if (this.id) {
                ...
                axios.post('http://127.0.0.1:8000/api/create_child_sub_menu', formData).then((res) => {
                    if (res.data.success) {
                        this.$notify({
        ...
        getSubMenuList() {
            axios.get('http://127.0.0.1:8000/api/get_all_sub_menu').then((res) => {
                if (res.data.success) {
                    console.log('res',res.data)
                    this.sub_menu_list = res.data.subMenu
                }
            }).catch((e) => {
                console.log('err', e)
            })
