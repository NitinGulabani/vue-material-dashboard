...
<div>
    ...
    <md-table v-model="sub_menu" :table-header-color="tableHeaderColor">
        <md-table-row slot="md-table-row" slot-scope="{ item }">
            <md-table-cell md-label="No">{{ item.id }}</md-table-cell>
            <md-table-cell md-label="Name">{{ item.sub_menu_name }}</md-table-cell>
    ...
    <h3 v-if="sub_menu.length == 0" class="text-center">No Records Found</h3>
    ...
    methods: {
        getAllRecords() {
            axios.get('http://127.0.0.1:8000/api/get_all_sub_menu').then((res) => {
                if (res.data.success) {
                    this.sub_menu = res.data.subMenu
                }
            }).catch((e) => {
                console.log('err', e)
            })
        ...
        deleteRecord(id) {
            swal({
                ...
                .then(async (willDelete) => {
                    if (willDelete) {
                        let sub_menu_data = await axios.post(`http://127.0.0.1:8000/api/delete_sub_menu/${id}`)
        ...
        redirect() {
            this.$router.push('/addsubmenu')
        },
        redirectEdit(id) {
            this.$router.push(`/addsubmenu/${id}`)
        }
    },
